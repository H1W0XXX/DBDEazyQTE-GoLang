...
KEYEVENTF_KEYUP=2
VK_SPACE=32
class MOUSEINPUT(ctypes.Structure):_fields_=[('dx',wintypes.LONG),('dy',wintypes.LONG),('mouseData',wintypes.DWORD),(_E,wintypes.DWORD),('time',wintypes.DWORD),(_F,ULONG_PTR)]
class KEYBDINPUT(ctypes.Structure):_fields_=[('wVk',wintypes.WORD),('wScan',wintypes.WORD),(_E,wintypes.DWORD),('time',wintypes.DWORD),(_F,ULONG_PTR)]
class _INPUTunion(ctypes.Union):_fields_=[('mi',MOUSEINPUT),('ki',KEYBDINPUT)]
class INPUT(ctypes.Structure):_anonymous_='u',;_fields_=[('type',wintypes.DWORD),('u',_INPUTunion)]
SendInput=ctypes.windll.user32.SendInput
SendInput.argtypes=wintypes.UINT,ctypes.POINTER(INPUT),ctypes.c_int
SendInput.restype=wintypes.UINT
def send_space():inp=INPUT();inp.type=INPUT_KEYBOARD;inp.ki=KEYBDINPUT(wVk=VK_SPACE,wScan=0,dwFlags=0,time=0,dwExtraInfo=0);SendInput(1,byref(inp),sizeof(INPUT));time.sleep(.01);inp.ki.dwFlags=KEYEVENTF_KEYUP;SendInput(1,byref(inp),sizeof(INPUT))
def sleep(t):
	st=time.time()
	while _A:
		offset=time.time()-st
...
def sleep_to(time_stamp):
	while _A:
		...
		if offset>=0:break
def win_screenshot(startw,starth,w,h):shot=_sct.grab({'left':startw,'top':starth,'width':w,_G:h});img=np.asarray(shot)[:,:,:3];return img[:,:,::-1]
...
def find_square(im_array):
	r_i=_D;shape=im_array.shape;target_points=[];global focus_level
...
def wiggle(t1,deg1,direction,im1):
	speed=wiggle_speed*direction;target1=270;target2=90;delta_deg1=(target1-deg1)%(direction*360);delta_deg2=(target2-deg1)%(direction*360);predict_time=min(delta_deg1/speed,delta_deg2/speed);print(_H,predict_time);click_time=t1+predict_time-press_and_release_delay+delay_degree/abs(speed);delta_t=click_time-time.time()
	if delta_t<0 and delta_t>-.1:send_space();print('quick space!!',delta_t,_C,speed);sleep(.13);return
	try:delta_t=click_time-time.time();sleep(delta_t);send_space();print(_I,delta_t,_C,speed);Image.fromarray(im1).save(imgdir+_J);sleep(.13)
...
def timer(im1,t1):
	...
	if delta_t<0 and delta_t>-.1:
		...
		return
	try:
		delta_t=click_time-time.time();checks_after_awake=0;checkwhen=0;im_array_pre_backup=_D
		while _A:
	...
	except ValueError as e:Image.fromarray(im1).save(imgdir+_J);print(e,delta_t,deg1,deg2,target)
def driver():
	global crop_w,crop_h,region;mon=_sct.monitors[1];screen_w,screen_h=mon['width'],mon[_G]
...
def bind_to_last_core():
	p=psutil.Process(os.getpid());cpu_count=os.cpu_count()
	if cpu_count is not _D and cpu_count>1:last_core=cpu_count-1;p.cpu_affinity([last_core]);print(f"已绑定到 CPU 核心 #{last_core}")
	else:print('CPU 核心不足，无法绑定')
def main():
	bind_to_last_core()
...
if __name__=='__main__':
	try:p=psutil.Process(os.getpid());p.nice(psutil.HIGH_PRIORITY_CLASS);print('已设置为高优先级')
	except Exception as e:print(f"无法设置优先级: {e}")
	main()
